...
import { Container, TopContainer, BottomContainer, Total, ViewCartBtn, CheckOutBtn, ProductInCart, DeleteBtn, ProductDetail } from "./styled";
          ...
          { cart && cart.length > 0 &&
            cart.map((item, index)=> (
              <ProductInCart key={`incart${index}`}>
                <ImageContainer image={item.image} width="25%" paddingTop="25%" marginRight="10px"/>
                <ProductDetail>
                  <Title>{`${item.title} `}<span>{`x ${item.count}`}</span></Title>
                  <Brand>{item.brand}</Brand>
                  <Price>{`$${item.price}.00`}</Price>
                </ProductDetail>
                <DeleteBtn onClick={() => removeFromCart(item.id)} data-testid={`removeFromCart${index}`}><i className="bi bi-x-lg"></i></DeleteBtn>
              </ProductInCart>
            ))
          }
        </TopContainer>
